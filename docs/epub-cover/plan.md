...
- 新增 `test_cover_page_with_image` — mock 下载成功，验证合成后的 cover.jpeg + OPF cover-image
...
4. 实现 `_compose_cover_jpeg(image, digest_date)` — 800×1200 白底画布 → 顶部绘制 RSSWise + 日期 → 下方居中放文章图 → JPEG 编码输出 `bytes`
