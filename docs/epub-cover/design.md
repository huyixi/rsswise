...
EPUB 摘要生成时新增封面页。封面页始终可用；当文章中存在可用封面图时，下载图片并合成为完整封面 JPEG：上方左对齐显示更大的 `RSSWise` 和生成日期，下方以更大的间距放置文章封面图。
...
- **图片处理**：Pillow 创建 800×1200 白底画布，左对齐绘制大号 `RSSWise` 和日期，将文章图等比缩放后居中放在下方，图片与日期的 gap 大于标题与日期的 gap，输出 JPEG（quality 85）。
- **下载工具**：使用生产依赖中的 `httpx`。
...
| COVER_TITLE_DATE_GAP | 24 px |
| COVER_IMAGE_GAP | 84 px |
